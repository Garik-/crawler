...
int
http_client(const ev_ares *eares, struct hostent *host) {
    ...
    ev_io_init(&con->io, connect_handler, sockfd, EV_WRITE);
...
follow_location(const ev_connect * con, const char *location, size_t len) {
    ...
    if (phr_parse_host(location, len, &host, &host_len) > 0) {
        ...
        host[host_len] = 0;
        ares_gethostbyname(con->eares->ares.channel, host, AF_INET, dns_callback, (void *) con->eares);
...
static void
configure_ares(ev_ares *eares, const struct ev_loop *loop) {
